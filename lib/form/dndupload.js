        ...
        initialise_queue: function(files) {
            ...
            for (i=0; i<files.length; i++) {
                if (this.options.maxbytes > 0 && files[i].size > this.options.maxbytes) {
                    ...
                    var maxbytesdisplay = this.options.maxbytes;
                    //This is snippet of code (with some changes) is from the display_size function in moodlelib
                    if (maxbytesdisplay >= 1073741824) {
                        maxbytesdisplay = (Math.round(maxbytesdisplay / 1073741824 * 10) / 10).toString().concat('GB');
                    } else if (maxbytesdisplay >= 1048576) {
                        maxbytesdisplay = (Math.round(maxbytesdisplay / 1048576 * 10) / 10).toString().concat('MB');
                    } else if (maxbytesdisplay >= 1024) {
                        maxbytesdisplay = (Math.round(maxbytesdisplay / 1024 * 10) / 10).toString().concat('KB');
                    }
                    var a = {file: files[i].name, size: maxbytesdisplay};
                    this.print_msg(M.util.get_string('maxbytesforfile', 'moodle', a), 'error');
