...
M.util.init_skiplink = function(Y) {
    var target;
    Y.all('a.skip').each(function() {
        this.on('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            target = this.getAttribute('data-target2');
            if (this.hasClass('skip-block')) {
                target = Y.one(target);
            } else {
                target = Y.one(target).ancestor('div').one('a');
            }
            if (!target) {
                target = Y.one(this.getAttribute('data-target1'));
                if (target.next('a')) {
                    target = target.next('a');
                } else {
                    while (true) {
                        target = target.ancestor();
                        if (target.next() && target.next().one('a')) {
                            target = target.next().one('a');
                            break;
                        }
                    }
                }
