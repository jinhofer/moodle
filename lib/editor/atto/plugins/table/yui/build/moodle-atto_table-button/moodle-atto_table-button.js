...
Y.namespace('M.atto_table').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
     ...
     *
     * @property _allowBorderStyle
     * @type Boolean
     * @private
     */
    _allowBorderStyle: null,

    /**
     * What border styles to allow
     *
     * @property _borderStyles
     * @type Array
     * @private
     */
    _borderStyles: null,
     ...
     *
     * @property _borderSize
     * @type Boolean
     * @private
     */
    _allowBorderSize: null,

    /**
     * Whether or not to allow colorizing borders
     *
     * @property _allowBorderColors
     * @type Boolean
     * @private
     */
    _allowBorderColors: null,

    /**
     * Whether or not to allow colorizing the background
     *
     * @property _allowBackgroundColor
     * @type Boolean
     * @private
     */
    _allowBackgroundColor: null,

    /**
     * The edit table template to be loaded
     *
     * @property _editTableTemplate
     * @type String
     * @private
     */
    _editTableTemplate: null,

    /**
     * The add table template to be loaded
     *
     * @property _addTableTemplate
     * @type String
     * @private
     */
    _addTableTemplate: null,
    ...
    initializer: function(param) {
        ...
        this._allowBorders = param.allowborder === '1';
        this._allowBorderStyle = param.allowborderstyles === '1';
        if (param.borderstyles) {
            this._borderStyles = param.borderstyles.replace(/ /g,'').split(';');
        } else {
            this._borderStyles = ['solid', 'dashed', 'dotted'];
        }
        this._allowBorderSize = param.allowbordersize === '1';
        this._allowBorderColors = param.allowbordercolor === '1';
        this._allowBackgroundColor = param.allowbackgroundcolor === '1';

        this._editTableTemplate = this._getTemplate(true);
        this._addTableTemplate = this._getTemplate(false);
    ...
    /**
     * Generate the templates based on current admin settings
     *
     * @method _getTemplate
     * @param {Boolean} edit
    ...
    _getTemplate: function(edit) {
        ...
        '<form class="{{CSS.FORM}}">' +
                ...
                '<br/>';
         if(!edit) {
            out += '<label for="{{elementid}}_atto_table_rows" class="sameline">{{get_string "numberofrows" component}}</label>' +
         ...
         if(this._allowBorders) {
            out += '<label for="{{elementid}}_atto_table_borders" class="sameline">{{get_string "borders" component}}</label>' +
                '<select class="{{CSS.BORDERS}}" id="{{elementid}}_atto_table_borders">' +
                    '<option value="none">{{get_string "none" component}}' + '</option>' +
                    '<option value="outer">{{get_string "outer" component}}' + '</option>' +
                    '<option value="all">{{get_string "all" component}}' + '</option>' +
                '</select>' +
                '<br>';
            if(this._allowBorderStyle) {
                out += '<label for="{{elementid}}_atto_table_borderstyle" class="sameline">' +
                       '{{get_string "borderstyle" component}}</label>' +
                       '<select class="{{CSS.BORDERSTYLE}}" id="{{elementid}}_atto_table_borderstyle">';
                     for(var i=0; i < this._borderStyles.length; i++) {
                         out += '<option value="' + this._borderStyles[i] + '">' + this._borderStyles[i] + '</option>';
                     }
                     out += '</select>' +
                     '<br>';
              }
              if(this._allowBorderSize) {
                  out += '<label for="{{elementid}}_atto_table_bordersize" class="sameline">' +
              ...
              if(this._allowBorderColors) {
                  out += '<label for="{{elementid}}_atto_table_bordercolor" class="sameline">' +
                         '{{get_string "bordercolor" component}}</label>' +
                         '<input id="{{elementid}}_atto_table_bordercolor" value="#000000" class="{{CSS.BORDERCOLOR}}"' +
                         'type="text" size="8"/>' +
                         '<br>';
              }
         }
         if(this._allowBackgroundColor) {
             out += '<label for"{{elementid}}_atto_table_backgroundcolor" class="sameline">' +
                    '{{get_string "backgroundcolor" component}}</label>' +
                    '<input id="{{elementid}}_atto_table_backgroundcolor" class="{{CSS.BACKGROUNDCOLOR}}"' +
                    'type="text" size="8"/>' +
                    '<br>';
         }
          out += '<div class="mdl-align">' +
                 '<br/>' +
    ...
    _getEditDialogueContent: function() {
        var template = Y.Handlebars.compile(this._editTableTemplate);
    ...
    _getDialogueContent: function() {
        var template = Y.Handlebars.compile(this._addTableTemplate);
        this._content = Y.Node.create(template({
                CSS: CSS,
                elementid: this.get('host').get('elementid'),
                component: COMPONENT
    ...
    _updateTable: function(e) {
        ...
        bordercolor = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BORDERCOLOR);
        borderstyle = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BORDERSTYLE);
        backgroundcolor = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BACKGROUNDCOLOR);

        if (bordercolor && bordercolor.get('value')[0] === '#') {
            borderhex = bordercolor.get('value');
        }
        ...
        if (backgroundcolor) {
            backgroundcolorvalue = backgroundcolor.get('value');
        }
        ...
        if (backgroundcolorvalue !== '' && backgroundcolorvalue[0] === '#') {
            table.setStyle('background-color', backgroundcolorvalue);
        }
        ...
        if (backgroundcolor) {
            if (bordertable !== '') {
                bordertable = bordertable.substring(0, bordertable.length-1) +
                              'background-color: ' + backgroundcolor.get('value') + ';"';
            }
            else {
                bordertable = ' style="background-color: ' + backgroundcolor.get('value') + ';"';
            }
        }
    ...
    _editTable: function() {
        ...
        var node = this._getEditDialogueContent(),
            ...
            bordercolor = node.one(SELECTORS.BORDERCOLOR),
            bordersize = node.one(SELECTORS.BORDERSIZE),
            backgroundcolor = node.one(SELECTORS.BACKGROUNDCOLOR),
            table = this._lastTarget.ancestor('table'),
            captionnode = table.one('caption');
        ...
        if(table.getStyle('background-color') !== 'transparent' && this._allowBackgroundColor) {
            backgroundcolor.set('value', Y.Color._rgbToHex(table.getStyle('background-color')));
        }

        if(this._allowBorders) {
            var bordervalue = 'none',
                bordercolorvalue = '#000',
                borderstylevalue = 'solid',
                bordersizevalue = '1',
                interest = null;
            if(table.getStyle('border') !== '0px none rgb(128, 128, 128)') {
                bordervalue = 'outer';
                interest = table;
            }
            else if(table.one('td').getStyle('border') !== '1px dashed rgb(187, 187, 187)') {
                bordervalue = 'all';
                interest = table.one('td');
            }

            if(interest) {
                bordercolorvalue = interest.getStyle('border-color');
                borderstylevalue = interest.getStyle('border-style');
                bordersizevalue = parseInt(interest.getStyle('border-width'), 10);
            }

            borderinput.set('value', bordervalue);

            if(borderstyle) {
                borderstyle.set('value', borderstylevalue);
            }
            if(bordersize) {
                bordersize.set('value', bordersizevalue);
            }
            if(bordercolor) {
                bordercolor.set('value', Y.Color._rgbToHex(bordercolorvalue));
            }
        }
