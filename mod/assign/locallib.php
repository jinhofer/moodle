...
class assign {
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                //STRY0010277 mart0969 20140414 - Update the submissions for all group members.
                $membersubmission = $this->get_user_submission($member->id, true, $submission->attemptnumber); //Change now creates a submission for each group member
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($completion->is_enabled($this->get_course_module()) && $instance->completionsubmit) {
                ...
                //STRY0010277 20140523 mart0969 - Update completion status for group members if this is a group submission
                if ($instance->teamsubmission) {
                    $team = $this->get_submission_group_members($submission->groupid, true);
                    foreach ($team as $member) {
