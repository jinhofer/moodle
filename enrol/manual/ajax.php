...
switch ($action) {
    ...
    case 'searchldap':
        ...
        // Most of this code is duplicated from the 'searchusers' case above except where noted.
        $enrolid = required_param('enrolid', PARAM_INT);
        $search = optional_param('search', '', PARAM_RAW);
        $page = optional_param('page', 0, PARAM_INT);
        $addedenrollment = optional_param('enrolcount', 0, PARAM_INT);
        $perpage = optional_param('perpage', 25, PARAM_INT);  //  This value is hard-coded to 25 in quickenrolment.js
        //UMN $outcome moved below since we need to create the user before generating the output.
        $extrafields = get_extra_user_fields($context);
        $useroptions = array();
